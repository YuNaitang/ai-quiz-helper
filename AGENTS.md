...
- Keep prompts and API request structure consistent with the current DeepSeek integration.
- Use `requests`, `flask`, `flask_cors`, and `python-dotenv` as the runtime dependencies.
- The backend supports caching for identical question/option/model combinations.
- Requests may include a `model` field to choose between `deepseek-chat` and `deepseek-reasoner`.
- The route now uses `httpx` async client with retry/backoff and request pooling for better throughput.
